...
for (gen_key, gen_data) in generated_gen
    ...
    for (edge_key, edge_data) in generated_edge_properties
        ...
        try
            result_ML_CCG = ML_accelerated_CCG(seed, first_stage_env, improved_stage_env,
                                                num_buses, num_gens, num_edges, len_of_features, pos_enc_k,
    ...
    ### save the results after each generator
    JSON.open(joinpath(parent_dir, "example_data/24-bus system/results", "final_ML_CCG_results_small.json"), "w") do io
        JSON.print(io, final_ML_CCG_wo_imp_results)
    end
    JSON.open(joinpath(parent_dir, "example_data/24-bus system/results", "time_ML_CCG_results_small.json"), "w") do io
        JSON.print(io, time_ML_CCG_wo_imp_results)
    end
end

############################################################################################################
##*************** 24-bus system model with 24 generators and 24 time periods *****************************##
##*************** ML acc-CCG method **********************************************************************##
